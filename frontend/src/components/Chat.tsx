...
import { Paperclip, FileText, Mic, Square } from 'lucide-react';
...
export const Chat = ({ ydoc, provider, onClose }: ChatProps) => {
    ...
    export const Chat = ({ ydoc, provider, onClose }: ChatProps) => {
        ...
        const isImage = (msg: Message) => {
            // Debug log for media detection
            if (msg.fileUrl) {
                console.log('Checking media:', {
                    fileName: msg.fileName,
                    fileType: msg.fileType,
                    url: msg.fileUrl,
                    isImage: msg.fileType?.startsWith('image/') || /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(msg.fileName || ''),
                    isAudio: msg.fileType?.startsWith('audio/') || /\.(webm|mp3|wav|m4a)$/i.test(msg.fileName || '')
                });
            }
